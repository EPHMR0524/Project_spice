...
def expand_line(line: str, functions: Func) -> str:
    ...
    line_expand = []
    func_name, *input_args, func = line[1:].split()
    params, body = functions[func]  # 取得調用子電路參數與內容

    # 將參數替換成輸入參數
    arg_map = dict(zip(params, input_args))
    for body_line in body:
        body_line = body_line.format(X_name=func_name).split()
        ...
        # 如果可能，繼續展開子電路
        line_expand.append(expand_line(body_line, functions))
    # 返回展開後的子電路內容
    return "\n".join(line_expand)
